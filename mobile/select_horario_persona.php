...
if($rol=="D"){
    $statement = mysqli_prepare($con,"SELECT fecha,clase.nombre,IFNULL(grupo,'Sin grupo'),hora_inicio,hora_final,IFNULL(observaciones,'Sin observaciones'),IFNULL(aula.nombre,'Sin aula asignada'),IF(id_docente=NULL,'Sin docente asignado',CONCAT(persona.nombre1,' ',persona.apellido1)) FROM clase JOIN clase_aula_horario ON id_clase=clase.id LEFT JOIN aula ON ((id_aula=aula.id)OR(id_aula=NULL)) LEFT JOIN persona ON ((id_docente=persona.id)OR(id_docente=NULL)) WHERE id_docente=? AND fecha=? ");
}else{
    ...
    mysqli_stmt_bind_result($statement, $fecha,$clase_nombre,$grupo,$hora_inicio,$hora_final,$observaciones,$aula_nombre,$docente);
    
    $response = array();
    
    while(mysqli_stmt_fetch($statement)){
        $response[] = array("fecha"=>$fecha,"clase"=>$clase_nombre,"grupo"=>$grupo,"hora_inicio"=>$hora_inicio,"hora_final"=>$hora_final,"observaciones"=>$observaciones,"aula"=>$aula_nombre,"docente"=>$docente);
    }
